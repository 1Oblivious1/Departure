...
- **Фронтенд** – разрабатывается на **React + Next.js**, что позволяет использовать серверный рендеринг и улучшать производительность.  
...
В рамках проектирования были созданы и документированы **UML-диаграммы**:  
...
- Внедрили **механизмы обработки ошибок** для предотвращения критических отказов.  
...
- Создан **API Swagger-документ**, упрощающий взаимодействие фронтенда с бэкендом.  
...
- Проверено соответствие функциональным и нефункциональным требованиям.  
...
- Разобрали **сложные моменты** и внесли небольшие исправления в проектирование API.  
...
- Провели внутреннюю презентацию, где объяснили **основные решения и их обоснование**.  
- Оценили, насколько архитектура **будет удобна для дальнейшей поддержки**.  

#### **3.3. Проверка отказоустойчивости**  
- Проанализировали, что **произойдет в случае сбоев базы данных**.  
- Проверили, как ведет себя система при **большом количестве одновременных запросов**.  
- Оценили механизм **логирования и обработки ошибок**, убедились, что критические сбои фиксируются и передаются в систему мониторинга.  
...
Теперь команда **может переходить к следующему этапу** – реализации кода на основе утвержденной архитектуры.
